...
/*   Updated: 2023/05/20 11:19:20 by rertzer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::executeForm(AForm & fm) const
{
	try
	{
		fm.execute(*this);
		std::cout << name << " executed " << fm.getName() << std::endl;
	}
	catch (std::exception &e)
	{
		std::cout << name << " couldn't execute " << fm.getName() << " because " << e.what() << std::endl;
	}
}
